...
export default function HistoryTable({history}: {history: any}) {
    ...
    const {setGroundFen, setGroundLastMoves, setGroundViewOnly, board} = useChessStore();
        ...
        <div className="overflow-y-auto h-[250px] overflow-x-hidden">
            {history &&
                history.map((move, index) => {
                    return (
                        <div key={index} className="w-full grid grid-cols-[1fr_16fr_16fr] gap-10 gap-y-[10px]">
                      <span>
                        {index + 1}
                          {".    "}
                      </span>
